  ...
  std::optional<std::reference_wrapper<std::vector<std::vector<double>>>> opt_dest)
{
  ...
  for (uint64_t gs = 0; gs < num_groupsets; ++gs)
  {
    ...
    const auto file_num_gs_angles =
      H5ReadDataset1D<uint64_t>(file_id, group_name + "/num_gs_angles");
    const auto file_num_gs_groups =
      H5ReadDataset1D<uint64_t>(file_id, group_name + "/num_gs_groups");
